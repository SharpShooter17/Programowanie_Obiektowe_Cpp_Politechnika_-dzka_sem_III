...
public:
	...
	RefMatrix		operator()(unsigned int row, unsigned int col)
	{
		return CMatrix<T>::RefMatrix(*this, row, col);
	}
...
inline CMatrix<T>::CMatrix() : 
	matrix_data(new MatrixDate(0, 0))
{
}

template <typename T>
CMatrix<T>::CMatrix(unsigned int row, unsigned int col) : 
	matrix_data( new MatrixDate(row, col))
{
}
...
template<typename T>
CMatrix<T>::CMatrix(std::string path_to_file) : matrix_data(new MatrixDate(0, 0))
{
	std::ifstream file;
...
CMatrix<T>::CMatrix(std::initializer_list<std::initializer_list<T>> lst) : 
	matrix_data(new MatrixDate(static_cast<unsigned int>(lst.size()), static_cast<unsigned int>(lst.begin()->size())))
{
	int j = 0;
...
CMatrix<T> CMatrix<T>::reverse() const
{
	...
	return ((CMatrix<T>(this->algebraicCoplement().transposition())) *= x);
}
...
inline std::ifstream & operator >> (std::ifstream & in, CMatrix<S> & A)
{
	...
	if (in.fail()) 
		throw::Matrix_Except::bad_matrix_file_format();
